...
public class SpringRest {
    ...
    private static final Logger logger = Logger.getLogger(SpringRest.class);
     ...
     // ADD BY SID 28.04.2016 10:26 ANZ TOWER JAKARTA 
     ...
     /*GET REST StationName from korean air  WEATHER API DATA*/
   //edited by sid 12.06.2016 05.34
     public static  String getStationname(String lat,String lng){
    	 final String uri = "http://apis.skplanetx.com/weather/current/hourly?lat="+lat+"&lon="+lng+"&version=1";
    	 String valueCity="";
         ...
         System.out.println(result);
         HashMap resultKey1 = (HashMap) result.getBody().get("weather");
         List<HashMap>  resultKey2 = (List) resultKey1.get("hourly");
         HashMap valueGrid = new HashMap<>();
         
         
         for (HashMap object : resultKey2) {
 			
 			if (object.containsKey("grid")) {
 				valueGrid = (HashMap) object.get("grid");
 				valueCity = (String) valueGrid.get("city");
 			}
 		}
         ...
         logger.info("=====================================================================");
         logger.info("Real time data hit to get city cluster"); 
         logger.info("Spring Rest Test Client sid  value city = "+valueCity);
         logger.info("======================================================================");
   
    	return valueCity;
     }
     
     //add by shiddiq 09.06.2016  
